...
export default function App() {
  ...
  return (
    <div className="min-h-screen font-sans text-black dark:text-white bg-gray-100 dark:bg-spotify-gray-500 transition-colors duration-300">
      <header className="p-4 flex justify-between items-center bg-white dark:bg-spotify-gray-400 shadow-md">
        <div className="flex items-center gap-3">
             <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-spotify-green"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
            <h1 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-white">Lyrics Card Generator</h1>
        </div>
        <button onClick={toggleTheme} className="p-2 rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-spotify-gray-300 transition-colors">
      ...
      <main className="p-4 md:p-8 max-w-4xl mx-auto">
        ...
        <div className="mt-8 bg-white dark:bg-spotify-gray-400 p-6 rounded-xl shadow-lg animate-fade-in">
          {error && <div className="bg-red-500 text-white p-3 rounded-md mb-4 text-center">{error}</div>}
          
          {renderStep()}

          {step > AppStep.SEARCH && (
             <div className="mt-6 pt-6 border-t border-gray-200 dark:border-spotify-gray-300 flex justify-between items-center">
              ...
              {step === AppStep.SELECT_LYRICS && (
                <button onClick={handleNextStep} disabled={selectedLyrics.length === 0} className="px-6 py-2 bg-spotify-green text-white font-bold rounded-full hover:bg-green-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  Continue
                </button>
              )}
            </div>
