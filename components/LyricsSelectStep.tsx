  ...
  return (
    <div>
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 dark:text-spotify-gray-100">{song.name} by {song.artists.map(a => a.name).join(', ')}</p>
      </div>
            ...
            <div
              ...
              className={`p-3 my-1 rounded-md cursor-pointer transition-all duration-200 text-center text-lg ${
                selectedLyrics.includes(line.line)
                  ? 'bg-spotify-green text-white scale-105'
                  : 'bg-gray-100 dark:bg-spotify-gray-300 hover:bg-gray-200 dark:hover:bg-spotify-gray-200'
              }`}
            >
              {line.line}
            </div>
